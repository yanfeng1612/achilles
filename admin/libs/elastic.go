...
func NewClient() {
	var err error
	client, err = elastic.NewClient(elastic.SetURL("http://192.168.2.77:9200"), elastic.SetBasicAuth("logstash_write", "logstash_write"))
	if err != nil {
		// Handle error
		panic(err)
	}
...
func Main() {
	...
	info, code, err := client.Ping("http://192.168.2.77:9200").Do(ctx)
	if err != nil {
		// Handle error
		panic(err)
	}
	...
	esversion, err := client.ElasticsearchVersion("http://192.168.2.77:9200")
	if err != nil {
		// Handle error
		panic(err)
	}
